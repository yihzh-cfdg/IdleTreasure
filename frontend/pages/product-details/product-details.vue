	...
	export default {
		...
		onLoad: function(opt) {
			if (opt.item === undefined)
				opt.item =
				'{"id":"1000000005","empName":"小华","price":20,"title":"钢笔","type":1,"image":"https://yihzh-imgbed.oss-cn-fuzhou.aliyuncs.com/image/avatar.jpg"}';
			const item = JSON.parse(decodeURIComponent(opt.item));
			console.log(item);
			this.goodId = item.id;
			this.userAvatar = item.avatar;
			this.userName = item.empName;
