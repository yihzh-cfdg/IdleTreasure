<template>
	<view class="container">
		...
		<view class="avatar-set">
			<uni-list-chat :avatar-circle="true" :title="userInfo.name" :avatar="userInfo.avatar"
				:note="userInfo.account" clickable link :to="'/pages/set/info-set?info='+jsoninfo">
...
<script>
	...
	export default {
		...
		onLoad:function(){
			...
			this.userInfo.avatar = user.header;
			this.userInfo.account = user.fzukey;
			this.jsoninfo = encodeURIComponent(JSON.stringify(this.userInfo));
		},
		methods: {
			...mapMutations(["logout"]),
			goBack() {
